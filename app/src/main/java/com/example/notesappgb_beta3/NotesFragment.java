...
public class NotesFragment extends Fragment implements View.OnClickListener {
    ...
    ArrayList<Note> notes = new ArrayList<>(Arrays.asList(new Note("Парикмахер", "Подстричься", "16.02.20"),
            new Note("Рабочая встреча", "Встреча с представителем банка", "18.02.20"), new Note("Прогулка", "Прогулка на велосипедах", "20.02.20")));

    private static TextView tvTitle1;
    private TextView tvTitle2;
    private TextView tvTitle3;
    private static TextView tvDescription1;
    private TextView tvDescription2;
    private TextView tvDescription3;
    private static TextView tvData1;
    ...
    private void openEmblemFragment(int index) {
        ...
        requireActivity().getSupportFragmentManager()
                .beginTransaction()
                .replace(R.id.layout_container2, fragment)
                .commit();
    }
    ...
    private void startNotesDetailActivity(int index) {
        if (!isLandscapeOrientation) {
            ...
            intent.putExtra(NotesDetailFragment.ARG_INDEX, index);
            intent.putExtra("title", notes.get(index).title);
            intent.putExtra("description", notes.get(index).description);
            intent.putExtra("data", notes.get(index).data);
